...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->Name = L"update";
			this->Text = L"update";
			this->Load += gcnew System::EventHandler(this, &passenger::update_Load);
	...
	private: System::Void button2_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		for (int i = 0; i < p.tickets.size(); i++)
		{
			...
			else if (p.tickets[i].passeneger_name == msclr::interop::marshal_as<std::string>(textBox5->Text))
			{
				...
				msclr::interop::marshal_as<std::string>(textBox1->Text) = t[i].passeneger_name;
				msclr::interop::marshal_as<std::string>(textBox2->Text) = t[i].boarding_point;
				msclr::interop::marshal_as<std::string>(textBox3->Text) = t[i].destination_point;
				msclr::interop::marshal_as<std::string>(textBox4->Text) = t[i].date_of_travel;
